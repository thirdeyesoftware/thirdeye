...
OPTIND=2
while getopts "frcFRCt:p:d:z:blDoqx:a:i:" OPTION
do
    case $OPTION in
        f)  fast=true;;
        r)  showMenu=false;;
        ...
        i)  infostring=,$OPTARG;;
        p)  project=$OPTARG;;
        x)  echo "proxyPort $OPTARG";proxy=true;proxyPort=$OPTARG;;
        lp) echo "localPort $OPTARG";localport=true;localPort=$OPTARG;;
        h)  help;exit 0;;
        \?) help;exit 1;;
    esac
done
...
select myinstance in $(cat $file | grep "$pattern" |env bluegreen=$DeploymentsType NormalEndingType=$NormalEndingType CASSEnding=$CASSEnding perl -F\\t -lane 'print if ($F[0] =~ m/$ENV{bluegreen}$ENV{NormalEndingType}/) || ($F[0] =~ m/$ENV{bluegreen}$ENV{CASSEnding}/) || $ENV{bluegreen} eq ""' |env myzones="$OVERRIDE_ZONES" perl -F\\t -lane 'print if $ENV{myzones} =~ m/$F[1]/') QUIT ;
  do
    ...
    if [ "$proxy" == "true" ]; then
      ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i ~/.ssh/google_compute_engine -o ProxyCommand="ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -i ~/.ssh/google_compute_engine -W %h:%p $bastion_host" -L $localPort:127.0.0.1:$proxyPort $instance_ip
    else
